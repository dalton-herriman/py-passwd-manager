...
@click.group()
@click.option('--vaults-dir', '-d', default='vaults', 
              help='Directory containing vaults')
@click.pass_context
...
def list_vaults(ctx):
    ...
    try:
        ...
        for vault in vaults:
            ...
            click.echo(f"Last accessed: {vault.last_accessed.strftime('%Y-%m-%d %H:%M:%S')}")
...
def delete_vault(ctx, name, force):
    ...
    if not force:
        if not click.confirm(f"Are you sure you want to delete vault '{name}'? This cannot be undone."):
