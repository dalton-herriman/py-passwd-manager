...
class MultiVaultPasswordManagerGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(
            vault_buttons_frame, text="Create Vault", command=self.create_vault
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(
            vault_buttons_frame, text="Delete Vault", command=self.delete_vault
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(
            vault_buttons_frame, text="Rename Vault", command=self.rename_vault
        ).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(
            vault_buttons_frame, text="Backup Vault", command=self.backup_vault
        ).pack(side=tk.LEFT, padx=(0, 5))
        ...
        # Vault scrollbar
        vault_scrollbar = ttk.Scrollbar(
            vaults_list_frame, orient=tk.VERTICAL, command=self.vaults_tree.yview
        )
        ...
        self.open_vault_btn = ttk.Button(
            entry_buttons_frame, text="Open Vault", command=self.open_vault
        )
        self.open_vault_btn.pack(side=tk.LEFT, padx=(0, 5))

        self.close_vault_btn = ttk.Button(
            entry_buttons_frame, text="Close Vault", command=self.close_vault
        )
        self.close_vault_btn.pack(side=tk.LEFT, padx=(0, 5))

        self.add_entry_btn = ttk.Button(
            entry_buttons_frame, text="Add Entry", command=self.add_entry
        )
        self.add_entry_btn.pack(side=tk.LEFT, padx=(0, 5))

        self.save_vault_btn = ttk.Button(
            entry_buttons_frame, text="Save Vault", command=self.save_vault
        )
        ...
        entry_scrollbar = ttk.Scrollbar(
            entries_list_frame, orient=tk.VERTICAL, command=self.entries_tree.yview
        )
        ...
        self.view_entry_btn = ttk.Button(
            entry_action_frame, text="View Entry", command=self.view_entry
        )
        self.view_entry_btn.pack(side=tk.LEFT, padx=(0, 5))

        self.edit_entry_btn = ttk.Button(
            entry_action_frame, text="Edit Entry", command=self.edit_entry
        )
        self.edit_entry_btn.pack(side=tk.LEFT, padx=(0, 5))

        self.delete_entry_btn = ttk.Button(
            entry_action_frame, text="Delete Entry", command=self.delete_entry
        )
        self.delete_entry_btn.pack(side=tk.LEFT, padx=(0, 5))

        self.copy_password_btn = ttk.Button(
            entry_action_frame, text="Copy Password", command=self.copy_password
        )
    ...
    def refresh_vaults_list(self):
        ...
        try:
            ...
            for vault in vaults:
                print(
                    f"DEBUG: Adding vault to tree: {vault.name} ({vault.entry_count} entries)"
                )
                status = (
                    "🔓 OPEN"
                    if self.vault_manager.get_current_vault_name() == vault.name
                    else "🔒 LOCKED"
                )
                values = (
                    vault.name,
        ...
        if messagebox.askyesno(
            ...
            try:
                ...
                if self.vault_manager.get_current_vault_name() == vault_name:
                    self.close_vault()
                messagebox.showinfo(
                    "Success", f"Vault '{vault_name}' deleted successfully!"
                )
            except Exception as e:
    ...
    def rename_vault(self):
        ...
        new_name = simpledialog.askstring(
            "Rename Vault", f"Enter new name for vault '{old_name}':"
        )
        if not new_name:
            return
    ...
    def backup_vault(self):
        ...
        if backup_path:
            try:
                ...
                messagebox.showinfo(
                    "Success", f"Vault '{vault_name}' backed up successfully!"
                )
            except Exception as e:
    ...
    def open_vault(self):
        ...
        password = simpledialog.askstring(
            "Open Vault", f"Enter master password for vault '{vault_name}':", show="*"
        )
        if not password:
            return
    ...
    def save_vault(self):
        ...
        password = simpledialog.askstring(
            "Save Vault", "Enter master password:", show="*"
        )
        if not password:
            return
        ...
        try:
            ...
            if current_vault_name:
                ...
                self.vault_manager.update_vault_entry_count(
                    current_vault_name, entry_count
                )
        ...
        if dialog.result:
            ...
            try:
                ...
                master_password = simpledialog.askstring(
                    "Save Vault", "Enter master password to save the vault:", show="*"
                )
                if master_password:
                    ...
                    if current_vault_name:
                        entry_count = len(
                            self.vault_manager.get_current_vault().get_entry()
                        )
                        self.vault_manager.update_vault_entry_count(
                            current_vault_name, entry_count
                        )
                    ...
                    messagebox.showwarning(
                        "Warning",
                        "Entry added but vault not saved. Use 'Save Vault' to persist changes.",
                    )
            except Exception as e:
                messagebox.showerror("Error", f"Failed to save vault: {str(e)}")
    ...
    def _complete_treeview_rebuild(self):
        ...
        try:
            ...
            if self.vault_manager.get_current_vault():
                ...
                print(
                    f"DEBUG: Rebuild completed - {len(self.entries_tree.get_children())} items"
                )
...
"""
    ...
    def edit_entry(self):
        ...
        if entries:
            ...
            dialog = EditEntryDialog(
                self.root, self.vault_manager.get_current_vault(), entry
            )
            if dialog.result:
                ...
                try:
                    ...
                    if master_password:
                        self.vault_manager.get_current_vault().save_vault(
                            master_password
                        )
                        # Update entry count in registry
                        current_vault_name = self.vault_manager.get_current_vault_name()
                        ...
                        messagebox.showwarning(
                            "Warning",
                            "Entry updated but vault not saved. Use 'Save Vault' to persist changes.",
                        )
                except Exception as e:
                    messagebox.showerror("Error", f"Failed to save vault: {str(e)}")
    ...
    def delete_entry(self):
        ...
        if messagebox.askyesno(
            "Confirm Delete", f"Are you sure you want to delete entry {entry_id}?"
        ):
            try:
                ...
                try:
                    ...
                    if master_password:
                        ...
                        messagebox.showwarning(
                            "Warning",
                            "Entry deleted but vault not saved. Use 'Save Vault' to persist changes.",
                        )
                except Exception as e:
                    messagebox.showerror("Error", f"Failed to save vault: {str(e)}")
    ...
    def generate_password(self):
        ...
        if dialog.result:
            ...
            if clipboard_handler(password):
                messagebox.showinfo(
                    "Success", f"Generated password copied to clipboard!\n\n{password}"
                )
            else:
                messagebox.showinfo(
                    "Generated Password", f"Generated password:\n\n{password}"
                )
...
class CreateVaultDialog:
    ...
    def setup_ui(self):
        ...
        ttk.Entry(main_frame, textvariable=self.confirm_var, show="*").grid(
        ...
        ttk.Entry(main_frame, textvariable=self.description_var).grid(
            row=3, column=1, sticky=(tk.W, tk.E), padx=(5, 0)
        )
...
class AddEntryDialog:
    ...
    def setup_ui(self):
        ...
        # Service
        ttk.Label(main_frame, text="Service:").grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        self.service_var = tk.StringVar()
        ttk.Entry(main_frame, textvariable=self.service_var).grid(
            row=0, column=1, sticky=(tk.W, tk.E), padx=(5, 0)
        )
    ...
    def add_entry(self):
        ...
        try:
            ...
            print(
                f"DEBUG: AddEntryDialog.add_entry - Entry created: {entry.name} (ID: {entry.id})"
            )
            self.result = entry
            self.dialog.destroy()
...
class GeneratePasswordDialog:
    ...
    def setup_ui(self):
        ...
        ttk.Spinbox(main_frame, from_=8, to=64, textvariable=self.length_var).grid(
            row=0, column=1, sticky=(tk.W, tk.E), padx=(5, 0)
        )
        ...
        ttk.Checkbutton(
            main_frame, text="Include symbols", variable=self.include_symbols_var
        ).grid(row=1, column=0, columnspan=2, sticky=tk.W, pady=5)

        self.include_numbers_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            main_frame, text="Include numbers", variable=self.include_numbers_var
        ).grid(row=2, column=0, columnspan=2, sticky=tk.W, pady=5)

        self.include_uppercase_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            main_frame, text="Include uppercase", variable=self.include_uppercase_var
