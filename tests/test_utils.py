...
class TestPasswordGeneration:
    ...
    def test_generate_password_various(self, length, include_symbols, include_numbers, include_uppercase):
        ...
        assert password != generate_password(length, include_symbols, include_numbers, include_uppercase)

    @pytest.mark.unit
    def test_generate_password_no_charset(self):
        """Test password generation with no character sets selected"""
        with pytest.raises(ValueError):
            generate_password(length=10, include_symbols=False, include_numbers=False, include_uppercase=False)
