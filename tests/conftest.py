...
def mock_crypto():
    ...
    with patch('pm_core.crypto.generate_salt') as mock_salt, \
         patch('pm_core.crypto.derive_key') as mock_derive, \
         patch('pm_core.crypto.encrypt_data') as mock_encrypt, \
         patch('pm_core.crypto.decrypt_data') as mock_decrypt:
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "crypto: Cryptography and security tests"
    )
    config.addinivalue_line(
        "markers", "gui: GUI-related tests"
    )
    config.addinivalue_line(
        "markers", "cli: CLI-related tests"
    )
    config.addinivalue_line(
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if any(keyword in item.nodeid.lower() for keyword in ['crypto', 'encrypt', 'decrypt', 'hash']):
            item.add_marker(pytest.mark.crypto)
        
        # Mark GUI-related tests
        if any(keyword in item.nodeid.lower() for keyword in ['gui', 'tkinter', 'widget']):
            item.add_marker(pytest.mark.gui)
        
        # Mark CLI-related tests
        if any(keyword in item.nodeid.lower() for keyword in ['cli', 'click', 'command']):
            item.add_marker(pytest.mark.cli)
        
        # Mark performance tests
        if any(keyword in item.nodeid.lower() for keyword in ['performance', 'benchmark', 'speed']):
            item.add_marker(pytest.mark.performance)
        
        # Mark security tests
        if any(keyword in item.nodeid.lower() for keyword in ['security', 'password', 'strength']):
