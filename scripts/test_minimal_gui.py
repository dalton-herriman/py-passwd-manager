...
class MinimalTestGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Button(button_frame, text="Create Vault", command=self.create_vault).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(button_frame, text="Open Vault", command=self.open_vault).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(button_frame, text="Add Entry", command=self.add_entry).pack(
            side=tk.LEFT, padx=(0, 5)
        )
        ttk.Button(button_frame, text="Refresh", command=self.refresh_entries).pack(
            side=tk.LEFT, padx=(0, 5)
        )
    ...
    def refresh_entries(self):
        ...
        try:
            ...
            for entry in entries:
                values = (entry.id, entry.name, entry.username or "", entry.url or "")
